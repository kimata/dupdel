...
def temp_cache_db(tmp_path):
    ...
    original_path = cache_module.CACHE_DB_PATH
    cache_module.CACHE_DB_PATH = str(tmp_path / "test_cache.db")
    yield cache_module.CACHE_DB_PATH
    cache_module.CACHE_DB_PATH = original_path
...
class TestCacheDb:
    ...
    def test_cache_pair(self, temp_cache_db):
        ...
        assert cache_module.get_cached_count() == 1
    ...
    def test_cache_pairs_bulk(self, temp_cache_db):
        ...
        assert saved == 3
        assert cache_module.get_cached_count() == 3
    ...
    def test_cache_pairs_bulk_empty(self, temp_cache_db):
        ...
        assert saved == 0
        assert cache_module.get_cached_count() == 0
