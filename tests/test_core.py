...
class TestExpandToDigitGroup:
    ...
    def test_single_digit(self):
        """単一の数字"""
        name = "test1file"
        start, end = _expand_to_digit_group(name, 4, 5)
        assert name[start:end] == "1"

    def test_multiple_digits(self):
        """複数桁の数字"""
        name = "test123file"
        start, end = _expand_to_digit_group(name, 5, 6)
        assert name[start:end] == "123"

    def test_no_expansion_needed(self):
        """拡張不要な場合"""
        name = "a1b"
        start, end = _expand_to_digit_group(name, 1, 2)
        assert name[start:end] == "1"
...
class TestFindDigitGroupInRange:
    ...
    def test_digit_in_range(self):
        """範囲内に数字がある場合"""
        name = "test123file"
        result = _find_digit_group_in_range(name, 4, 7)
    ...
    def test_no_digit_in_range(self):
        """範囲内に数字がない場合"""
        name = "testfile"
        result = _find_digit_group_in_range(name, 0, 4)
        assert result is None

    def test_digit_with_space(self):
        """数字とスペースが混在する範囲"""
        name = "#2 (test)"
        result = _find_digit_group_in_range(name, 1, 3)
        assert result is not None
...
class TestHasZengoDiff:
    ...
    def test_zengo_diff_mae_ato(self):
        """「前」と「後」の差分"""
        assert _has_zengo_diff("番組名 前編", "番組名 後編") is True

    def test_zengo_diff_ato_mae(self):
        """「後」と「前」の差分（逆順）"""
        assert _has_zengo_diff("番組名 後編", "番組名 前編") is True
...
class TestHasEpisodeNumberDiff:
    ...
    def test_single_digit_episode(self):
        ...
        assert _has_episode_number_diff(name1, name2) is True
    ...
    def test_date_diff_not_episode(self):
        ...
        assert _has_episode_number_diff(name1, name2) is False
...
class TestGetMtimeSafe:
    ...
    def test_existing_file(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            mtime = _get_mtime_safe(f.name)
            assert mtime > 0

    def test_nonexistent_file(self):
        """存在しないファイル"""
        mtime = _get_mtime_safe("/nonexistent/file.txt")
        assert mtime == 0
